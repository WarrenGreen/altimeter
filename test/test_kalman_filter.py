...
def test_update():
    ...
    assert KalmanFilter.update(57, 9, 119, 2) == (107.72727272727273, 1.6363636363636362)
    assert KalmanFilter.update(46, 1, 108, 3) == (61.5, 0.75)
    assert KalmanFilter.update(20, 9, 120, 5) == (84.28571428571429, 3.2142857142857144)
    assert KalmanFilter.update(69, 5, 92, 3) == (83.375, 1.875)
    assert KalmanFilter.update(56, 2, 111, 3) == (78.0, 1.2000000000000002)
    assert KalmanFilter.update(95, 9, 59, 4) == (70.07692307692308, 2.769230769230769)
    ...
    with open(str(DATA / "linear_flight.jsonl"), "r") as f:
        for line in f:
            ...
            if "gps_altitude" in sample:
                ...
                mu, sigma = KalmanFilter.update(mu, sigma, gps_altitude, gps_sigma)
            barometer_altitude = barometer.altitude(sample["pressure"])
            mu, sigma = KalmanFilter.update(mu, sigma, barometer_altitude, barometer.variance)
            ...
            pred_mu, pred_sigma = KalmanFilter.predict(mu, sigma, delta, PREDICTION_SIGMA)
            prev_mu = mu
            mu = pred_mu
            sigma = pred_sigma
