...
def main():
    ...
    barometer_start = (
        random.randrange(87 * 10, 108 * 10, 1) / 10.0
    )  # min and max records for sea level pressure
    altitude_start = 1524
    barometer = Barometer()
    with open(output_filepath, "w") as f:
        for altitude in range(altitude_start, 2134, 1):
            ...
            barometer_variance = (
                random.randrange(
                    -barometer_variance_max * 100, barometer_variance_max * 100, 1
                )
                / 100.0
            )
            barometer_pressure = pressure + barometer_variance
            gps_variance = (
                random.randrange(-gps_variance_max * 10, gps_variance_max * 10, 1)
                / 10.0
                + 0.001
            )
